...
def main():
    ...
    Encoder_para_idx = [str(i) for i in range(0, 15)]
    Det_Head_para_idx = [str(i) for i in range(15, 23)]
    Da_Seg_Head_para_idx = [str(i) for i in range(23, 31)]
    Ll_Seg_Head_para_idx = [str(i) for i in range(31, 39)]
    ...
    for epoch in range(begin_epoch+1, cfg.TRAIN.END_EPOCH+1):
        ...
        if rank in [-1, 0]:
            ...
            torch.save(model, 'C:/Github/YOLOP/weights/yolov8-p.pt')
            save_checkpoint(
                epoch=epoch,
                name=cfg.MODEL.NAME,
                model=model,
